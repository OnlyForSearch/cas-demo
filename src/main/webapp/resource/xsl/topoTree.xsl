...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:IE="www.ibs.com">
  ...
  <!--缺省的叶节点-->
  <xsl:variable name="defaultIco" select="'chart.gif'"/>
  <!--缺省的父节点-->
    ...
    <html>
      ...
      <body oncontextmenu="cancelRightClicked();bodyRight.show()" onselectstart="return false" ondrag="return false" onclick="cancelRightClicked()">
        ...
        <IE:PopupMenu id="bodyRight" width="130">
          ...
          <item label="刷新" event="refresh()" ico="refresh.gif"/>
        </IE:PopupMenu>
    ...
    <xsl:choose>
      <xsl:when test="error_code=0">
        ...
        <script>
          ...
          var defaultParentIco = "<xsl:value-of select="$defaultParentIco"/>";
          var newItemIco = new Array(defaultParentIco,defaultIco);
          iniPage();
         </script>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template match="Menu">
    ...
    <IE:PopupMenu id="itemRight" width="130">
      ...
      <item ico="new03.gif" label="新建">
        <item ico="new03.gif" label="拓扑图分类" event="addTopoTreeItem(0)"/>
        <item ico="chart.gif" label="拓扑图" event="addTopoTreeItem(1)"/>
      </item>
                  ...
                  <tr valign="middle">
                    ...
                    <td width="100%">
                      <nobr class="MenuItem" onMouseOver="itemOver()" onMouseOut="itemOut()">
                        <xsl:if test="@type=1">
                        	<xsl:attribute name="id">
                          		<xsl:value-of select="concat('chart_',@moduleID)"/>
                        	</xsl:attribute>
                        </xsl:if>
