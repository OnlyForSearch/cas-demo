...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:IE="www.ibs.com">
    ...
    <html>
      <head>
        ...
        <style>
          ...
          @import url(../resource/css/netElement.css);
        ...
        <script src="../resource/js/netElement.js"/>
        <script src="../resource/js/Dialog.js"/>
        <script src="../resource/js/Error.js"/>
        <title>菜单</title>
      </head>
      <body oncontextmenu="cancelRightClicked();bodyRight.show()" onselectstart="return false" ondrag="return false" onclick="cancelRightClicked()" onmousemove="drag()" onmouseup="endDrag()">
        <!--右键菜单配置-->
  ...
  <xsl:template match="Menu">
    <!--右键菜单配置-->
    <IE:PopupMenu id="itemRight" width="180">
      <xsl:attribute name="icoUrl">
        <xsl:value-of select="$IcoUrl"/>
      </xsl:attribute>
      <item ico="new03.gif" label="添加" event="addNetElement(2)"/>
      <item ico="edit.gif" label="重命名" event="rename()"/>
      <item ico="del.gif" label="删除" event="delMenuItem()"/>
      <item ico="properties.gif" label="属性" event="editNetElement()"/>
    </IE:PopupMenu>
    <!--生成树形菜单-->
    <div class="MenuDIV" id="treeDIV">
      ...
      <table border="0" cellpadding="0" cellspacing="0" id="treeTable" width="100%">
        ...
        <tr height="100%" id="topoTreeTR" style="display:none">
        	<td><iframe frameborder="0" scrolling="NO" height="100%" width="100%" name="oChartFrame"/></td>
        </tr>
      </table>
    </div>
  </xsl:template>
  ...
  <xsl:template name="outMenu">
    ...
    <xsl:for-each select="node()">
      <xsl:sort select="@moduleID" data-type="number"/>
      <tr>
        <td>
                  ...
                  <tr valign="middle">
                    ...
                    <td width="100%">
                      <nobr class="MenuItem" onMouseOver="itemOver()" onMouseOut="itemOut()">
                        ...
                        <xsl:attribute name="neTypeId">
                          <xsl:value-of select="@neTypeId"/>
                        </xsl:attribute>
                        ...
                        <xsl:if test="$depth!=1">
                        	<xsl:attribute name="oncontextmenu">
                          		<xsl:value-of select="'menuItemRight()'"/>
                        	</xsl:attribute>
                        </xsl:if>
