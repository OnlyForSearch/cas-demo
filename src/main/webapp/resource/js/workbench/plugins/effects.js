...
Element.collectTextNodes = function(a) {
	return $A($(a).childNodes).collect(function(b) {
		return (b.nodeType == 3 ? b.nodeValue : (b.hasChildNodes() ? Element
				.collectTextNodes(b) : ""))
	}).flatten().join("")
};
Element.collectTextNodesIgnoreClass = function(a, b) {
	return $A($(a).childNodes).collect(function(c) {
		return (c.nodeType == 3 ? c.nodeValue : ((c.hasChildNodes() && !Element
				.hasClassName(c, b)) ? Element
...
Effect.ScopedQueue = Class.create(Enumerable, {
			...
			add : function(b) {
				...
				var a = Object.isString(b.options.queue)
						? b.options.queue
		...
		this.render = (function() {
			...
			return function(c) {
				...
				if (this.state === "running") {
					c = (this.options.transition(c) * this.fromToDelta)
							+ this.options.from;
					this.position = c;
					b(this, "beforeUpdate");
					if (this.update) {
...
Effect.Tween = Class.create(Effect.Base, {
			initialize : function(c, f, e) {
				...
				var b = $A(arguments), d = b.last(), a = b.length == 5
						? b[3]
						: null;
...
Effect.Move = Class.create(Effect.Base, {
			...
			update : function(a) {
				this.element.setStyle({
							left : (this.options.x * a + this.originalLeft)
									.round()
									+ "px",
							top : (this.options.y * a + this.originalTop)
									.round()
									+ "px"
						})
			}
...
Effect.Scale = Class.create(Effect.Base, {
			...
			setup : function() {
				...
				if (this.options.scaleMode == "box") {
					this.dims = [this.element.offsetHeight,
							this.element.offsetWidth]
				}
				if (/^content/.test(this.options.scaleMode)) {
					this.dims = [this.element.scrollHeight,
...
Effect.Highlight = Class.create(Effect.Base, {
			...
			setup : function() {
				...
				this._base = $R(0, 2).map(function(a) {
					return parseInt(this.options.startcolor.slice(a * 2 + 1, a
											* 2 + 3), 16)
				}.bind(this));
				this._delta = $R(0, 2).map(function(a) {
					return parseInt(this.options.endcolor.slice(a * 2 + 1, a
											* 2 + 3), 16)
							- this._base[a]
				}.bind(this))
			},
...
Effect.ScrollTo = function(c) {
	var b = arguments[1] || {}, a = document.viewport.getScrollOffsets(), d = $(c)
			.cumulativeOffset();
	if (b.offset) {
		d[1] += b.offset
	}
...
Effect.Appear = function(b) {
	b = $(b);
	var a = Object.extend({
				from : (b.getStyle("display") == "none" ? 0 : b.getOpacity()
						|| 0),
				to : 1,
...
Effect.DropOut = function(b) {
	...
	return new Effect.Parallel([new Effect.Move(b, {
						...
						afterFinishInternal : function(c) {
							c.effects[0].element.hide().undoPositioned()
									.setStyle(a)
						}
					}, arguments[1] || {}))
};
	...
	return new Effect.Scale(c, 100, Object.extend({
						...
						afterUpdateInternal : function(d) {
							d.element.down().setStyle({
								bottom : (d.dims[0] - d.element.clientHeight)
										+ "px"
							})
						},
						afterFinishInternal : function(d) {
...
Effect.Shrink = function(c) {
	...
	return new Effect.Parallel([new Effect.Opacity(c, {
						...
						afterFinishInternal : function(g) {
							g.effects[0].element.hide().undoClipping()
...
Effect.Pulsate = function(c) {
	...
	var b = arguments[1] || {}, a = c.getInlineOpacity(), e = b.transition
		...
		return 1
				- e((-Math.cos((f * (b.pulses || 5) * 2) * Math.PI) / 2) + 0.5)
	};
	...
	update : function(a) {
		...
		while (c--) {
			d[(b = this.transforms[c]).style] = b.unit == "color"
					? "#"
							+ (Math.round(b.originalValue[0]
									+ (b.targetValue[0] - b.originalValue[0])
									* a)).toColorPart()
							+ (Math.round(b.originalValue[1]
									+ (b.targetValue[1] - b.originalValue[1])
									* a)).toColorPart()
							+ (Math.round(b.originalValue[2]
									+ (b.targetValue[2] - b.originalValue[2])
									* a)).toColorPart()
					: (b.originalValue + (b.targetValue - b.originalValue) * a)
							.toFixed(3)
							+ (b.unit === null ? "" : b.unit)
		}
...
Effect.Transform = Class.create({
			...
			play : function() {
				return new Effect.Parallel(this.tracks.map(function(a) {
							var d = a.get("ids"), c = a.get("effect"), b = a
									.get("options");
							var e = [$(d) || $$(d)].flatten();
...
$w("fade appear grow shrink fold blindUp blindDown slideUp slideDown pulsate shake puff squish switchOff dropOut")
		.each(function(a) {
...
$w("getInlineOpacity forceRerendering setContentZoom collectTextNodes collectTextNodesIgnoreClass getStyles")
		.each(function(a) {
