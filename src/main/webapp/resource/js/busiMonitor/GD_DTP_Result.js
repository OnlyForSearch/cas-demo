		...
		getValueCfgById		: function(cfgId)
		{
			...
			else
			{
				...
				if (window.ActiveXObject)
			    {
					...
					{
						oReturn = sendRequest.responseXML.selectSingleNode("/root/value_cfg");
					}
			    }
				else
			  	{
			        oReturn = selectSingleNode(sendRequest.responseXML, "/root/value_cfg");
			  	}
			}
			return oReturn;
		},
...
Result = (function()
{
	...
	Class.methods({
				...
				{
					var o = selectSingleNode(oElement ,path);
					return (o) ? getXmlText(o) : defaultValue;
				},
	...
	Class.prototype = {
		...
		{
			this.paramList = [];
			var params = selectNodes(oXml, this.getParamXpath);
		...
		buildSendXml		: function(params, options)
		{
			...
			for (var i = 0, op; op = this.paramList[i]; i++)
			{
				...
				for (var c in send)
				{
					if (c == "value")
					{
						np = setNodeText(np, this.getParamValue(send[c], op, name, options.comp));
					}
					else
					{
		...
		readyStateChange	: function()
		{
			if (this.sendRequest.readyState == 4)
			{
    			...
    			else
    			{
        			...
        			this.resultXml = parser.parseFromString(this.sendRequest.responseText, "text/xml");
    			}
