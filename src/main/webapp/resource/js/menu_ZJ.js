...
{  
	...
	window.parent.location.replace(sMainPageURL);
}
...
function doMenu_loadTree(frameName,mainFrame,mainUrl)
{
    ...
    var sSecondPageURL="indexFrame_ZJ.jsp";
	var menuId="-1";
    var arrayUrl=getURLSearch();
    if(arrayUrl!=null)
    {
	...
	parent.window.location.replace(sSecondPageURL+"?id="+oSelectedItem.id+"&mainUrl="+encodeURIComponent(mainLoadUrl)+"&menuId="+menuId);
}
...
function doMenuReplace(frameName,mainFrame,mainUrl)
{
	...
	var defaultWelComePage = 'welcome.jsp?name='+encodeURIComponent(trimall(oSelectedItem.innerText));
	var mainLoadUrl = oSelectedItem.CUSTOM_URL_NAME || (mainUrl || defaultWelComePage);
	parent.window.location.replace(sSecondPageURL+"?id=2050&mainUrl="+encodeURIComponent(mainLoadUrl)+"&menuId="+menuId);
}
...
{	
	...
	if(oSelectedItem.SERVER_URL_NAME)
	{
        window.parent.document.all(iFrame).src = url;		
		//window.open(url,oSelectedItem.id,sFeatures.join(","));
	}
}
function doMenu_loadWin(frameName)
{
	window.parent.document.all(frameName).src = oSelectedItem.SERVER_URL_NAME;
}
//删除快捷菜单
