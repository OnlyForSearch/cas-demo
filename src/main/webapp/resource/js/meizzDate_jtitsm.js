...
strFrame='<style>';
strFrame+='INPUT.button{BORDER-RIGHT: #5F82D3 1px solid;BORDER-TOP: #5F82D3 1px solid;BORDER-LEFT: #5F82D3 1px solid;';
strFrame+='BORDER-BOTTOM: #5F82D3 1px solid;BACKGROUND-COLOR: #fff8ec;font-family:宋体;}';
...
strFrame+='<table border=1 cellspacing=0 cellpadding=0 width=142 height=160 bordercolor=#5F82D3 bgcolor=#5F82D3 Author="wayx">';
...
strFrame+='      <tr align=center Author="wayx"><td width=16 align=center bgcolor=#5F82D3 style="font-size:12px;cursor: hand;color: #ffffff" ';
...
strFrame+='        <td width=16 bgcolor=#5F82D3 align=center style="font-size:12px;cursor: hand;color: #ffffff" ';
...
strFrame+='<table border=1 cellspacing=0 cellpadding=0 bgcolor=#5F82D3 ' + (bMoveable? 'onmousedown="DragStart()" onmouseup="DragEnd()"':'');
strFrame+=' BORDERCOLORLIGHT=#5F82D3 BORDERCOLORDARK=#FFFFFF width=140 height=20 Author="wayx" style="cursor:' + (bMoveable ? 'move':'default') + '">';
...
strFrame+='    <table border=1 cellspacing=2 cellpadding=0 BORDERCOLORLIGHT=#5F82D3 BORDERCOLORDARK=#FFFFFF bgcolor=#fff8ec width=140 height=120 Author="wayx">';
var n=0; for (j=0;j<5;j++){ strFrame+= ' <tr align=center Author="wayx">'; 
for (i=0;i<7;i++){
	if(shike){
		strFrame+='<td width=20 height=20 id=meizzDay'+n+' style="font-size:12px" Author=meizz></td>';
	}else{
		strFrame+='<td width=20 height=20 id=meizzDay'+n+' style="font-size:12px" Author=meizz ondblclick="parent.meizzDayClick(this.innerText,0)"></td>';
	}
	n++;
}
strFrame+='</tr>';}
strFrame+='      <tr align=center Author="wayx">';
for (i=35;i<37;i++)strFrame+='<td width=20 height=20 id=meizzDay'+i+' style="font-size:12px" Author=wayx ></td>';
//strFrame+='        <td colspan=3 align=right Author=meizz><span onclick="parent.meizzDayClick(this.innerText,0)" style="font-size:11px;cursor: hand" align="center"><u>确定</u></span>&nbsp;<span onclick=parent.closeLayer() style="font-size:11px;cursor: hand" align="center"';
strFrame+='        <td colspan=5 align=right Author=meizz><span onclick="parent.meizzDayClick(this.innerText,0)" style="font-size:12px;color:red;cursor: hand" align="center"><u>确定</u></span>&nbsp;<span onclick="parent.valueClean()" style="font-size:12px;color:red;cursor: hand" align="center"><u>清空</u></span>&nbsp;<span onclick=parent.closeLayer() style="font-size:12px;color:red;cursor: hand" align="center"';
strFrame+='         Author=meizz ><u>关闭</u></span></td></tr>';
strFrame+='    </table></td></tr><tr Author="wayx"><td Author="wayx">';
strFrame+='        <table id="sfm" border=0 cellspacing=1 cellpadding=0 width=100% Author="wayx" bgcolor=#FFFFFF style="display:none">';
strFrame+='          <tr Author="wayx"><td align=center>时间:</td><td Author=meizz align=left><select id=shi style="font-size: 11px">';
for (i=0;i<24;i++)   strFrame+='<option value="'+((i<10)?'0'+i:i)+'">'+((i<10)?'0'+i:i)+'</option>';
strFrame+='          </select></td><td align=center>点</td>';
strFrame+='             <td Author=meizz align=right><select id=fen style="font-size: 11px"> ';
for (i=0;i<60;i++)   strFrame+='<option value="'+((i<10)?'0'+i:i)+'">'+((i<10)?'0'+i:i)+'</option>';
strFrame+='             </select></td><td align=center>分</td> ';
strFrame+='</tr></table>';
strFrame+='<table id="normalpan" border=0 cellspacing=1 cellpadding=0 width=100% Author="wayx" bgcolor=#FFFFFF>';
...
var shike=false;		//存放日历对象
function setday(tt,obj,shikeflag) //主调函数
{
	...
	dads.display = '';
	odatelayer.shi.value="00";
	odatelayer.fen.value="00";
//	odatelayer.miao.value="00";

	if (shikeflag) {		
	   shike=true;
	   odatelayer.normalpan.style.display="none";
	   odatelayer.sfm.style.display=""
       if(r!=null){
		   odatelayer.shi.value=((r[4]<10)&&(r[4]>0)&&(r[4].length<2))?"0"+r[4]:r[4];
		   odatelayer.fen.value=r[5];
	   }else{
		   odatelayer.shi.value=(new Date().getHours()<10)?"0"+new Date().getHours():new Date().getHours();
		   odatelayer.fen.value=(new Date().getMinutes()<10)?"0"+new Date().getMinutes():new Date().getMinutes();
	   }
	}
	else {
	   shike=false;
	   odatelayer.normalpan.style.display="";
	   odatelayer.sfm.style.display="none";
	}
...
{ 
  with(window.event)
  { if (srcElement.Author==null && srcElement != outObject && srcElement != outButton)
    closeLayer();
  }
}
 ...
 document.onkeyup=function()		//按Esc键关闭，切换焦点关闭
  {
    if (window.event.keyCode==27){
		...
		if(document.activeElement.Author==null && document.activeElement != outObject && document.activeElement != outButton)
		{
			closeLayer();
		}
  }
...
function meizzToday()  //Today Button
  {
    ...
    yuefen=(meizzTheMonth<10)? ("0"+meizzTheMonth): (""+meizzTheMonth);
    today=new Date().getDate();
    rizi=(today<10)? ("0"+today): (""+today);
    //meizzSetDay(meizzTheYear,meizzTheMonth);
    if(outObject){
	outObject.value=meizzTheYear + "-" + yuefen + "-" + rizi;
    }
    closeLayer();
  }
...
function meizzSetDay(yy,mm)   //主要的写程序**********
{
  ...
  { var da = eval("odatelayer.meizzDay"+i)     //书写新的一个月的日期星期排列
    if (meizzWDay[i]!="")
      { 
		//初始化边框
		da.borderColorLight="#5F82D3";
		da.borderColorDark="#FFFFFF";
		if(i<firstday)		//上个月的部分
		{
			da.innerHTML="<b><font color=gray>" + meizzWDay[i] + "</font></b>";
			da.title=(mm==1?12:mm-1) +"月" + meizzWDay[i] + "日";
			da.onclick=Function("cjwPassVal(this.innerText,-1,"+i+")");
			if(!outDate){
				da.style.backgroundColor = ((mm==1?yy-1:yy) == new Date().getFullYear() && 
					(mm==1?12:mm-1) == new Date().getMonth()+1 && meizzWDay[i] == new Date().getDate()) ?
					 "#FFD700":"#e0e0e0";
				//Author:linjl 2010-07-09
		        if((yy == new Date().getFullYear() && mm == new Date().getMonth()+1 && meizzWDay[i] == new Date().getDate())){
		            cjwTheDay=meizzWDay[i];
                    cjwMonEx=0;
                    selday = eval("odatelayer.meizzDay"+i);
		        }
		    }else{
		...
		else		//本月的部分
		{
			...
			da.onclick=Function("cjwPassVal(this.innerText,0,"+i+")");		//给td赋予onclick事件的处理
...
var cjwTheDay=new Date().getDate(),cjwMonEx="",selday=null;
function cjwPassVal(n,ex,i) {
  cjwTheDay=n;
  cjwMonEx=ex;
  ...
  if(selday){
     //灰色
     //Author:linjl 2010-07-09
     if(selday.outerText!=n){
         selday.style.backgroundColor="#e0e0e0";
     }
  }
  selday=da;
}

function valueClean() //清空
{
	outObject.value = "";
	closeLayer();
}

function setTimes(sYear,sMonth,sDay)
{
	if(outObject==null){
		sMonth = parseInt(sMonth,10);
		sDay = parseInt(sDay,10);
		var outObj=sYear+"-"+((sMonth<10)?"0"+sMonth:sMonth)+"-"+((sDay<10)?"0"+sDay:sDay);
		return outObj;
	}
...
function meizzDayClick(n,ex)  //点击显示框选取日期，主输入函数*************
{
  ...
  var mm = parseInt(meizzTheMonth)+cjwMonEx;	
	//判断月份，并进行对应的处理
	if(mm<1){
		yy--;
		mm=12+mm;
  ...
  if (outObject)
  {
    if (!n) {//outObject.value=""; 
      return;}
    if ( (cjwTheDay < 10) && (cjwTheDay.length=1)){cjwTheDay = "0" + cjwTheDay;}
    outObject.value= yy + "-" + mm + "-" + cjwTheDay ; //注：在这里你可以输出改成你想要的格式
    if (shike) {
       outObject.value=outObject.value+" "+odatelayer.shi.value+":"+odatelayer.fen.value+":00";   //+":"+odatelayer.miao.value;
    }
    closeLayer(); 
  }
