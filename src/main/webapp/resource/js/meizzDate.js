...
strFrame+='<td width=20 height=20 id=meizzDay'+n+' style="font-size:12px" Author=meizz ></td>';n++;}
strFrame+='</tr>';}
strFrame+='      <tr align=center Author="wayx">';
for (i=35;i<39;i++)strFrame+='<td width=20 height=20 id=meizzDay'+i+' style="font-size:12px" Author=wayx ></td>';
//modify by syj 把确认改为清空
...
strFrame+='        <td colspan=3 align=right Author=meizz><span onclick="parent.meizzDayClear()" style="font-size:11px;cursor: hand" align="center"><u>清空</u></span>&nbsp;<span onclick=parent.closeLayer() style="font-size:11px;cursor: hand" align="center"';
...
strFrame+='          <tr Author="wayx"><td Author=meizz align=left><select id=shi style="font-size: 11px">';
for (i=0;i<24;i++)   strFrame+='<option value="'+((i<10)?'0'+i:i)+'">'+((i<10)?'0'+i:i)+'</option>';
strFrame+='          </select></td><td align=center><b>:</td>';
...
strFrame+='             </select></td><td align=center><b>:</td> ';
strFrame+='             <td Author=meizz align=right><select id=miao style="font-size: 11px"> ';
for (i=0;i<60;i++)   strFrame+='<option value="'+((i<10)?'0'+i:i)+'">'+((i<10)?'0'+i:i)+'</option>';
strFrame+='             </select></td>';
strFrame+='</tr></table>';
...
function setday(tt,obj,shikeflag,pobj) //主调函数
{
	...
	var vWid=event.clientX+obj.clientWidth;
	
	if(typeof(pobj)!='undefined'&&pobj!=document.body){
		ttop = ttop - pobj.scrollTop;
	}
	
	while (tt = tt.offsetParent){ttop+=tt.offsetTop; tleft+=tt.offsetLeft;}
	if(bhei-ttop+btop<211){
		ttop = ttop - 211-thei;
	}
	...
	if (shikeflag) {
	   ...
	   if(r!=null){
		   ...
		   odatelayer.miao.value=(new Date().getSeconds()<10)?"0"+new Date().getSeconds():new Date().getSeconds();
	   }
	}
	else {
	   shike=false;
    ...
    if (meizzWDay[i]!="")
      { 
		...
		if(i<firstday)		//上个月的部分
		{
			...
			da.ondblclick=onDbclickFunction;//2009-12-31 扩展双击选中事件
			if(!outDate)
...
var cjwTheDay="",cjwMonEx="",selday=null;
...
function onDbclickFunction(){
 ...
 var yy=meizzTheYear;
  ...
  if (outObject)
  {
    if ( cjwTheDay < 10 && cjwTheDay.length < 2){cjwTheDay = "0" + cjwTheDay;}
    outObject.value= yy + "-" + mm + "-" + cjwTheDay ; //注：在这里你可以输出改成你想要的格式
    if (shike) {
       outObject.value=outObject.value+" "+odatelayer.shi.value+":"+odatelayer.fen.value+":"+odatelayer.miao.value;
    }
    closeLayer(); 
  }
}
function meizzDayClick(n,ex)  //点击显示框选取日期，主输入函数*************
{
  ...
  if (outObject)
  {
    if (!n) {//outObject.value=""; 
      return;}
    //当不选择日期时先提示
	if(cjwTheDay<1){alert("请先选择日期!");return;}
    if ( cjwTheDay < 10 && cjwTheDay.length < 2){cjwTheDay = "0" + cjwTheDay;}
    outObject.value= yy + "-" + mm + "-" + cjwTheDay ; //注：在这里你可以输出改成你想要的格式
    if (shike) {
       outObject.value=outObject.value+" "+odatelayer.shi.value+":"+odatelayer.fen.value+":"+odatelayer.miao.value;
