...
function loadBillInfos(){
    ...
    var actionUrl = "servlet/billInfoServlet?tag=41&recent=10";//tag=1
    ...
    if(billXmlhttp!=null && billXmlhttp.readyState==4 && isSuccess(billXmlhttp))
    {
        ...
        if(hasPrivilege=="true"){
            ...
            for(var i=0;i<iLen&&i<8;i++)
            {
                ...
                if(i==0){
                    firstBillOutput += "<span style='color:#2067A9;font-size:13px;'>";
                    firstBillOutput += "<a onclick=\"showBillInfoWindow(3,'"+oRows[i].selectSingleNode('BILL_INFO_ID').text+"','"+oRows[i].selectSingleNode("CATEGORY_NAME").text+"')\" style='font-size:15px;color:#2166A9;line-height:22px;' ";
                    ...
                    firstBillOutput += "<br/><font style='color:#999999'> "+ oRows[i].selectSingleNode("SUBMIT_DATE").text +"</font>";
                    firstBillOutput += "</span>";
                }else{
...
function setMsgInfo(){
    ...
    var xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
    xmlhttp.onreadystatechange=function(){renderTODOList(xmlhttp);renderToTrackList(xmlhttp);}
...
function setKbase(){
    ...
    if (isSuccess(xmlhttp)){       
        ...
        for(var i=0;i<oRows.length;i++){
        	...
        	//var link = '/workshop/knowledge/knowledgeRead.html?kId='+kId+'&state=4SA&configName=KNOWLEDGES&IS_PRI_UPDATE=true&IS_PRI_DELETE=true&IS_PRI_AUDIT=true&IS_PRI_STORAGE=true';
...
function WorkLink(){
    ...
    xmlRequest.Open("POST","servlet/MainPageAction?action=1&LINK_TYPE=D",true);
    xmlRequest.send();  
}
...
function OperationAnalysis(){
    ...
    xmlRequest.onreadystatechange=function(){
        if(xmlRequest.readyState==4){
            OperationAnalysisMenuLoad(xmlRequest)
        }
    }
    xmlRequest.Open("POST","servlet/MainPageAction?action=8",true);
    xmlRequest.send();  
}
function OperationAnalysisMenuLoad(xmlRequest){
    if(isSuccess(xmlRequest)){
        var mMenus = xmlRequest.responseXML.selectNodes("/root/MENU");
        if(mMenus.length >0){
        	document.getElementById("OADiv").style.display="block";
        	var mIsFirst = true;
            for(var i=0;i<mMenus.length;i++){
            	//一级：
                var mMenuId = getNodeValue(mMenus[i],'MENU_ID');
                var mMenuName = getNodeValue(mMenus[i],'MENU_NAME');
                var mLinkType = getNodeValue(mMenus[i],'LINK_TYPE');
                var mLinkValue = getNodeValue(mMenus[i],'LINK_VALUE');
                var mHasPrivilegeStaffs = getNodeValue(mMenus[i],'HAS_PRIVILEGE_STAFFS');
                var mPStaffs = ","+mHasPrivilegeStaffs+",";
                var curStaffId = ","+cur_StaffId+",";
                if(mPStaffs.indexOf(curStaffId) == "-1"){
                	continue;
                }
                var className = "ni_rightItem_def";
                if(mIsFirst){
                	className = "ni_rightItem_sel";
                }
				var mMenuInHTML = '<span class="'+className+'" mNm="'+mMenuId+'" onMouseOver="clickDbMainTab(this)" onMouseOver="this.style.color=\'#FF0000\'" onMouseOut="outMainTable(this)" onMouseOut="this.style.color=\'\'" >'+mMenuName+'</span>';
        		document.getElementById("OAMainTab").insertAdjacentHTML("beforeEnd",mMenuInHTML); //一级TAB
            	
            	var cMenuInHTML = '';
            	var cConInHTML = '';
            	var cMenus = mMenus[i].selectNodes("MENU");
        		if(cMenus.length >0){
            		//有二级：
        			var cIsFirst = true;
            		for(var c=0;c<cMenus.length;c++){
                		var cMenuId = getNodeValue(cMenus[c],'MENU_ID');
                		var cMenuName = getNodeValue(cMenus[c],'MENU_NAME');
                		var cLinkType = getNodeValue(cMenus[c],'LINK_TYPE');
                		var cLinkValue = getNodeValue(cMenus[c],'LINK_VALUE');
                		var cHasPrivilegeStaffs = getNodeValue(cMenus[c],'HAS_PRIVILEGE_STAFFS');
                		var cPStaffs = ","+cHasPrivilegeStaffs+",";
                		if(cPStaffs.indexOf(curStaffId) == "-1"){
                			continue;
                		}
                		var className = "dbDef";
                		var styleStr = "";
                		if(cIsFirst){
                			className = "dbSel";
                			styleStr  = 'border-top:1px solid #B4E1F3;';
                		}
						cMenuInHTML += '<div class="'+className+'" mNm="'+mMenuId+'" cNm="'+cMenuId+'" onMouseOver="clickDbTab(this);">';
						cMenuInHTML += '	<div onMouseOver="this.children[0].style.color=\'#F00\';" onMouseOut="this.children[0].style.color=\'\';" style="'+styleStr+'"><font class="dbTitle">'+cMenuName+'</font></div>';
						cMenuInHTML += '</div>';
                		...
                		var cInHtml = "";
                		if(cLinkType=="WORKLINK"){
                			cInHtml = getWorkLinkConfigBymenuId(cMenuId);
                		}else if(cLinkType=="URL"){
                			cInHtml = '<iframe src="'+cLinkValue+'" width="100%" height="100%" frameborder="0" framespacing="0" scrolling="no"></iframe>';
                		}else if(cLinkType=="HTML"){
                			cInHtml = cLinkValue;
                		}
						cConInHTML += '<div id="OAConList_'+mMenuId+'_con_'+cMenuId+'" style="display:'+displays+';margin:0;padding:0;height:275px; padding-top:20px;">'+cInHtml+'</div>';
            			cIsFirst = false;
            		}
        		
        			var mConInHTML = '';
        			var oHeight = 0;
        			if(7 - cMenus.length >0){
        				oHeight = 30*(7-cMenus.length);
        			}
        			var className = "OACon_def";
                	if(mIsFirst){
                		className = "OACon_sel";
                	}
					mConInHTML +='<div id="OACon_'+mMenuId+'" class="'+className+'">';
					mConInHTML +='	<table width="100%" border="0" cellspacing="0" cellpadding="0">';
					mConInHTML +='	  <tr>';
					mConInHTML +='	    <td width="135px" valign="top">';
					mConInHTML +='				<div class="dbTop"></div>';
					mConInHTML +='				<div id="OAConTab_'+mMenuId+'"">';
					mConInHTML +=					cMenuInHTML;
					mConInHTML +='				</div>';
					mConInHTML +='				<div class="dbLine"><div></div></div>';
					mConInHTML +='				<div class="dbLine" style="height:'+oHeight+'px;"></div>';
					mConInHTML +='				<div class="dbBottom"></div>';
					mConInHTML +='	    </td>';
					mConInHTML +='	    <td valign="top" id="OAConList_'+mMenuId+'"">';
					mConInHTML +=			cConInHTML;
					mConInHTML +='	    </td>';
					mConInHTML +='	  </tr>';
					mConInHTML +='	</table>';
					mConInHTML +='</div>';
        			document.getElementById("OACon").insertAdjacentHTML("beforeEnd",mConInHTML);        		
        		}else{
            		//无二级：
                	var mInHtml = "";
                	if(mLinkType=="WORKLINK"){
                		mInHtml = getWorkLinkConfigBymenuId(mMenuId);
                	}else if(mLinkType=="URL"){
                		mInHtml = '<iframe src="'+mLinkValue+'" width="100%" height="100%" frameborder="0" framespacing="0" scrolling="no"></iframe>';
                	}else if(mLinkType=="HTML"){
                		mInHtml = mLinkValue;
                	}
					...
					mConInHTML +='<div id="OACon_'+mMenuId+'" class="'+className+'" style="padding-top:10px;">';
					mConInHTML +=	mInHtml;
					mConInHTML +='</div>';
        			document.getElementById("OACon").insertAdjacentHTML("beforeEnd",mConInHTML);     
        		}
...
function getWorkLinkConfigBymenuId(menuId){
	var xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
    var submitURL = "servlet/MainPageAction?action=9&pLinkId="+menuId;
    xmlhttp.Open("GET",submitURL,false);
    xmlhttp.send();
    var workLinkDivHTML = '';
    if(isSuccess(xmlhttp)){
        var LinkIds = xmlhttp.responseXML.selectNodes("/root/rowSet");
        if(LinkIds.length >0){
            for(var i=0;i<LinkIds.length;i++)
            {
                linkVal = getNodeValue(LinkIds[i],'LINK_VALUE');
                linkImg = getNodeValue(LinkIds[i],'LINK_IMG');
                linkTitle = getNodeValue(LinkIds[i],'LINK_TITLE');
                remark = getNodeValue(LinkIds[i],'REMARK');
                if(remark && remark!="null" && remark!="undefined" && remark!=""){
					workLinkDivHTML += '<span title="'+remark+'">';
                }else{
					workLinkDivHTML += '<span>';
				}
				workLinkDivHTML += '<a class="ni_FX" style="background-image:url('+linkImg+');" target="_blank" href="'+linkVal+'">';
				workLinkDivHTML += '<br style="line-height:38px;"/>';
				workLinkDivHTML += '<span class="titleFont">'+linkTitle+'</span>';
				workLinkDivHTML += '</a>';
				workLinkDivHTML += '</span>';
            }
        }
    }
    return workLinkDivHTML;
}

function clickDbMainTab(clickObj){
	var mNm = clickObj.getAttribute("mNm");
	var tabDiv = document.getElementById("OAMainTab");
	var spanList= tabDiv.children;
	for(var i = 0; i<spanList.length; i++){
		if(spanList[i].className=="ni_rightItem_sel"){
			spanList[i].className = "ni_rightItem_def";
		}
	}
	clickObj.className = "ni_rightItem_sel";
	
	var conDiv = document.getElementById("OACon");
	var conList= conDiv.children;
	for(var i = 0; i<conList.length; i++){
		if(conList[i].className=="OACon_sel"){
			conList[i].className = "OACon_def";
		}
	}
	document.getElementById("OACon_"+mNm).className="OACon_sel";
	if(document.getElementById("OADiv1")){
		var o = document.getElementById("OADiv");
		var oTop = o.offsetTop+o.offsetHeight;
		var oLeft = o.offsetLeft;
		var div = document.getElementById("OADiv1");
		div.style.top = oTop;
...
function outMainTable(outObj){
	if(document.getElementById("OADiv1")){
		var myID =event.toElement.parentElement.id;
		...
		if(myID== "OADiv1"){
			document.getElementById("OADiv1").style.display = "block";
			var tabDiv = document.getElementById("OAMainTab");
			var spanList= tabDiv.children;
			for(var i = 0; i<spanList.length; i++){
				if(spanList[i].className=="ni_rightItem_sel"){
			...
			outObj.className = "ni_rightItem_sel";
		}else{
			document.getElementById("OADiv1").style.display = "none";
			hideOacon();
		}
		
		$("#OADiv1").mouseover(function () {
			document.getElementById("OADiv1").style.display = "block";
			var tabDiv = document.getElementById("OAMainTab");
			var spanList= tabDiv.children;
			for(var i = 0; i<spanList.length; i++){
...
function clickDbTab(clickObj){
	var cNm = clickObj.getAttribute("cNm");
	var mNm = clickObj.getAttribute("mNm");
	
	var tabDiv = document.getElementById("OAConTab_"+mNm);
	var tabList= tabDiv.children;
	for(var i = 0; i<tabList.length; i++){
		if(tabList[i].className=="dbSel"){
			tabList[i].className = "dbDef";
		}
	}
	clickObj.className = "dbSel";
	
	var conDiv = document.getElementById("OAConList_"+mNm);
	var conList= conDiv.children;
	for(var i = 0; i<conList.length; i++){
		conList[i].style.display = "none";
	}
	document.getElementById("OAConList_"+mNm+"_con_"+cNm).style.display="block";
}

function hasYyfxPrivilege(){
	var sql = "select strcat(has_privilege_staffs) as TEXT from CUST_JTITSM_INDEX_YYFX_MENU where parent_menu_id = '-1'";
	var staffIds = queryData(sql);
   	staffIds = ","+staffIds+",";
    var curStaffId = ","+cur_StaffId+",";
   	if(staffIds.indexOf(curStaffId) > -1){
		return true;
	}else{
		return false;
	}
