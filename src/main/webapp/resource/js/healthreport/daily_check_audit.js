...
daily.check.gridPanel = function(config) {
	...
	store = new Ext.data.Store({
				...
				proxy : new Ext.data.HttpProxy({
							//linjl 2012-3-14
							//action_type=audit 维护作业审核
							url : '../../servlet/healthRptAction?action=31&action_type=audit'
						}),
				reader : jsR,
				remoteSort : true
	...
	daily.check.gridPanel.superclass.constructor.call(this, {
				...
				sm : sm,
				// plugins: [this.groupG],
				tbar : [this.queryDailyBtn,'-',this.putinJobBtn],
				viewConfig : {
					forceFit : false,
					rowSelectorDepth : 50
...
Ext.extend(daily.check.gridPanel, Ext.grid.GridPanel, {
	...
	putinJobFn:function() {
		...
		**/
	    ...
	    //var resultArr = window.showModalDialog("../maintjobplan/itemResultFillIn.html",params,"resizable=yes;dialogWidth="+iWidth+";dialogHeight="+iHeight+";help=0;scroll=0;status=0;");
	    
		//linjl 2012-3-15
		params.push(window);
		params.push(decodeURIComponent(Global.url.jobname));  //作业名称
		params.push(Global.url.jobinstanceid);  //作业实例ID
		params.push('');  //审核实例ID
		params.push(Global.url.date);  //日期
		params.push('1');
		params.push(1);
		...
		if(!window.dialogArguments
			.checkIsComplete(Global.url.jobinstanceid)){
			return;
		}
	    var resultArr = window.showModalDialog("../../../workshop/maintjobplan/auditResultFillIn.html"
	    	,params,"resizable=yes;dialogWidth="+iWidth+";dialogHeight="+iHeight+";help=0;scroll=0;status=0;");
		//end
	...
	onRowDblClickFn : function(grid, rowIndex, e) {
		...
		//linjl 2012-3-15 维护作业审批
		params.action_type = 'audit';
		//end
		window
				.showModelessDialog(
