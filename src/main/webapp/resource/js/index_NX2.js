...
function refeshDaiban(){
	...
	loadPendingList("NX_DAIBAN","pendingList","1");
	...
	loadPendingList("NX_YUEBAN","readList","4");
}
...
function boardInfos(){
	...
	if(getHasPrivilege("906301", staffId)=="false"){//查看
		...
		window.setTimeout("document.getElementById('boardInfo').src = 'workshop/board/boardScroll_NX.htm'",10);//公告栏滚动boardScroll_JT.htm boardScroll.jsp
...
function addTopMenu_NOC(id,oMenuBar,curMenuId)
{
	var i;
	var menuId="-1";
    var arrayUrl=getURLSearch();
    var aMenuBg=getaMenuBg_NOC();
    if(arrayUrl!=null)
    {
	   menuId = arrayUrl.menuId;
	   if(typeof(arrayUrl.action)!='undefined'
	...
	if(isSuccess(xmlhttp))
	{
	    ...
	    if(menuId!="-1"&&action!=1)
	    {
	        outHTML +=  '<span style="cursor:hand;float:left" onmouseover="doMenuOver_NOC(this)" '
	                    +    'onmouseout="doMenuOut_NOC(this)" onclick="doMenu_refresh()" ' 
	                    +    'class="'+aMenuBg.menuNormalClass+'">'
	                    +  '主页'
	                    +'</span>';
	    }
		xmlDoc.load(xmlhttp.responseXML);
		var oNodes = xmlDoc.selectNodes("/root/rowSet");
		var iLen=oNodes.length;
		for(i=0;i<iLen;i++)
		{
		    var oNode=oNodes[i];
		    ...
		    var isCurMenu=(curMenuId==oNode.getAttribute("PRIVILEGE_ID"));
		    var menuClass=(isCurMenu)?aMenuBg.menuSeledClass:aMenuBg.menuNormalClass;
		    if(i!=0){
				outHTML +=   '<span class="menuLine">|</span>';		    	
		    }
			outHTML +=   '<span style="cursor:hand;float:left"'
			outHTML +=    ' id="'+oNode.getAttribute("PRIVILEGE_ID")+'"';
			//outHTML +=    ' oncontextmenu="rightClickedMenuItem=this;itemRight_add.show()"'
			...
			if(!isCurMenu)
			{
			   outHTML +=    ' onmouseover="doMenuOver_NOC(this)" onmouseout="doMenuOut_NOC(this)"';
			}
			outHTML +=   ' class="'+menuClass+'">'+oNode.getAttribute("PRIVILEGE_NAME");
	        outHTML +='</span>';
...
function getaMenuBg_NOC(){
    var aMenuBg=
    {
      menuNormalClass:'menu_NOC',
      menuSelClass:'SelMenu_NOC',
      menuSeledClass:'SeledMenu_NOC'
    }
    return aMenuBg;
}
function doMenuOver_NOC(oMenuItem){
   var aMenuBg=getaMenuBg_NOC();
   if(oMenuItem.className != aMenuBg.menuSeledClass){
   		oMenuItem.className=aMenuBg.menuSelClass;
   }
}
function doMenuOut_NOC(oMenuItem){
   var aMenuBg=getaMenuBg_NOC();
   if(oMenuItem.className != aMenuBg.menuSeledClass){
...
function setWorkLinkHbZhuanti(linkNodes)
{
	...
	for(var i=0;i<linkNodes.length;i++)
	{
		...
		//var linkImg2 ="<img width='60px' height='50px' src='" + linkImg + "' alt='" + linkTitle + "' border='0' />";  
		var linkImg2 ="<img width='40px' height='40px' src='" + linkImg + "' alt='" + linkTitle + "' border='0' />";  
		...
		//var temp='<a href="'+linkVal+'" style="float:left; height:55px;width:240px;margin:1px 1% 1px 3%;" class="zhuantiACSS">';
		var temp='<a href="'+linkVal+'" style="float:left; height:40px;width:230px;margin:2px 1% 2px 3%;" class="zhuantiACSS">';
		if(checkEd=="0"){//权限判断 没有权限链接无效
			...
			//temp='<a href="javascript:OpenLink()" style="float:left; height:55px;width:240px;margin:1px 1% 1px 3%;" class="zhuantiACSS">'
			temp='<a href="javascript:OpenLink()" style="float:left; height:40px;width:230px;margin:2px 1% 2px 3%;" class="zhuantiACSS">'
		}
		outHTML += temp
	...
	pendingList.onLoad = function(oXml)
	{
		...
		for(var i=0; i<loopTimes; i++) {
			...
			if(num=="1" && pendingInfo.is_read=="0")
				...
				pendingTitle = "<div style='float:left;padding-left:10px;font-family:Microsoft YaHei;width:93%;font-size:13px;'><a href='javascript:openSingle(" + i+","+num + ")'class='daibanACss' title='"+pendingInfo.content+"'>"+new_state+""+"【"+pendingInfo.type_name+"】"+pendingInfo.content+"</a></div>";
			else if(num == "1")
	        	pendingTitle = "<div style='float:left;padding-left:10px;font-family:Microsoft YaHei;width:90%;font-size:13px;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='javascript:openSingle(" + i+","+num + ")'class='daibanACss' title='"+pendingInfo.content+"'>"+"【"+pendingInfo.type_name+"】" + pendingInfo.content+"</a></div>";
	        else
	        	pendingTitle = "<div style='float:left;padding-left:10px;font-family:Microsoft YaHei;width:90%;font-size:13px;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='javascript:openSingle(" + i+","+num + ")'class='daibanACss' title='"+pendingInfo.content+"'>" + pendingInfo.content+"</a></div>";
	        /*
	        if(num=="1"||num=="4")
			...
			*/
			...
			aPendingCell.innerHTML =pendingTitle+alert_img;
		}
		//判断是哪种代办
		if(num=="1"){
			//var d = ResultFactory.newResult("GANSU_DAIBAN_COUNT");
			var d = ResultFactory.newResult("NX_DAIBAN_COUNT");
		...
		*/
		if(num=="4"){
			...
			document.getElementById("readPageNum").innerHTML = '阅办('+rowsNum+')';
		}
