...
Ext.extend(Ext.ux.TreeCheckNodeUI, Ext.tree.TreeNodeUI, {
    renderElements : function(n, a, targetNode, bulkRender){
        ...
        var checkable = true;
        if(tree.CheckableCondition){
        	checkable = eval(tree.CheckableCondition.replace(/:/gi, 'a.'));
        }
		var cb = (this.onlyLeafCheckable && a.leaf) || checkable; //(!this.onlyLeafCheckable || (a.leaf && checkable))
	...
	childCheck : function(node){
		...
		if(!a.leaf) {
			...
			for(var i = 0; i < cs.length; i++) {
				csui = cs[i].getUI();
				if(csui.checkbox && (csui.checkbox.checked ^ a.checked))
	...
	childHasChecked : function(node){
		...
		if(childNodes || childNodes.length>0){
			for(var i=0;i<childNodes.length;i++){
				if(childNodes[i].getUI().checkbox && childNodes[i].getUI().checkbox.checked)
					return true;
			}
		}
		return false;
