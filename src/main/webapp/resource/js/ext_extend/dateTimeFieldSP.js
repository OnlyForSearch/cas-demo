...
//add for datetimepicker
        ...
        if(this.hourel.options.length>0){ 
        	if(value.getHours() > this.maxHour){
        		this.hourel.selectedIndex = this.maxHour - this.minHour;
        	}else if(value.getHours()<this.minHour){
        		this.hourel.selectedIndex = 0;
        	}else {
        		this.hourel.selectedIndex = value.getHours() - this.minHour;
        	}
        }
       	if(this.minuteel.options.length>0){
       		 this.minuteel.selectedIndex = Math.floor(value.getMinutes()/this.minuteScale);
       	}
        this.value = value;
        if(this.el){
    ...
    onRender : function(container, position){
    	if(this.minHour < 0 ) this.minHour=0;
        if(this.maxHour > 23 ) this.maxHour=23;
    	if(this.minHour>this.maxHour) {
        	var hTmp = this.maxHour;
        ...
        if(this.minuteScale <1 ) this.minuteScale=1;
        //if(this.minuteScale >60 ) this.minuteScale=60; 超过就不管了
...
//DateTime add start
        ...
        m[m.length] = '<table cellspacing="0"><tr><td class="x-date-hour"></td><td>:</td><td class="x-date-minute"></td></tr></table>';
        m[m.length] = '</td></tr>';
//DateTime add end
        ...
        //add double click
        this.eventEl.on("dblclick", this.handleDateDoubleClick,  this, {delegate: "a.x-date-date"});
...
//add start 
        ...
        tdhourel.appendChild(this.hourel);
    		...
    		if(this.value.getHours() > this.maxHour){
        		this.hourel.selectedIndex = this.maxHour - this.minHour;
        	}else if(this.value.getHours()<this.minHour){
        		this.hourel.selectedIndex = 0;
        	}else {
        		this.hourel.selectedIndex = this.value.getHours() - this.minHour;
        	}
        ...
        tdminuteel.appendChild(this.minuteel);
        	for(var i=0;i<60;i+=this.minuteScale){
        ...
        this.minuteel.options[Math.floor(this.value.getMinutes()/this.minuteScale)].selected=true;
...
//add end
    ...
    handleDateDoubleClick : function(e, t){
        e.stopEvent();
        if(t.dateValue && !Ext.fly(t.parentNode).hasClass("x-date-disabled")){
        	var dt = new Date(t.dateValue);
     	    dt.setHours(this.hourel.options[this.hourel.selectedIndex].value);
