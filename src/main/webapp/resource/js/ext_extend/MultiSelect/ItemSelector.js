...
Ext.ux.form.ItemSelector = Ext.extend(Ext.form.Field,  {
    ...
    fromTo : function() {
    	if(!this.fireEvent('beforeAdd', this, this.getValue(), this.hiddenField.dom.value))return;
    ...
    toFrom : function() {
    	if(!this.fireEvent('beforeDelete', this, this.getValue(), this.hiddenField.dom.value))return;
        var selectionsArray = this.toMultiselect.view.getSelectedIndexes();
        var records = [];
        if (selectionsArray.length > 0) {
            for (var i=0; i<selectionsArray.length; i++) {
