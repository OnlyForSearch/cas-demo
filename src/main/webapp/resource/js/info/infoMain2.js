...
function initContent()
{
    ...
    //document.getElementById("contentIframe").src = 'editBillInfoContent.htm?iniUrl='+iniUrl+'&billInfoId='+billInfoId+'&tempDirectory='+billInfoForm.tempDirectory.value;
...
//    /resource/ewebeditor/ewebeditor.htm?id=content&style=itnm&tmpDir=knowledgeImgs
}
...
function submitBillInfo(form, ifClose)
{
    ...
    //console.log("{========" + document.getElementById("contentEditor").contentWindow.eWebEditor.window.document.contentWindow.getHTML());
    //console.log("{========" + document.getElementById("content").contentWindow.eWebEditor.window.document.contentWindow.getHTML());
    //console.log("{========2" + content.window);
  //var doc=document.getElementById("content").contentWindow.eWebEditor;
  //var doc= document.getElementById("content").contentWindow.eWebEditor.window.document.documentElement.outerHTML;
  //  console.log("{========2" + document.getElementById("content").contentWindow.eWebEditor.contentWindow.getHTML());
  isClose = ifClose;
  //(1). 基本信息
  ...
  try{
	...
	if(document.readyState=="complete")
	{
		...
		else
		{
            ...
            if(billInfoId==0){
              //contentIframe.window.frames["templateBillInfo"].myEditor.document.body.innerHTML = billInfoForm.content.value;
                document.getElementById("contentIframe").contentWindow.eWebEditor.window.document.body.innerHTML = billInfoForm.content.value;
...
function setBlank(form)
{
    ...
    form.source.value = "";
    //(2). 发布内容
    document.getElementById("contentIframe").contentWindow.eWebEditor.window.document.documentElement.innerHTML="";
    //(3). 关键字
    var iLen = form.keyword.length;
    var oOption;
    for(var i=iLen-1;i>=0;i--){
