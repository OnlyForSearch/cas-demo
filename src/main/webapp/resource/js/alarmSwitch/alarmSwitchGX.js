...
function doSearch() {
    ...
    var searchNode = addNewNode(rootNode, "search", '', new Array('pagesize', 'page', 'orderby'), new Array('15', '1', ' A.STATE,A.SEQ '));
    ...
    //规则类型为集中告警传送规则，值为1
    addNewNode(searchNode, "param", 1, new Array('fieldName', 'oper', 'type'), new Array('A.TRANS_RULE_TYPE', '=', 'string'));
    
   	document.all.oData.sendXML=sendXML.xml;
    document.all.oData.xmlSrc= url + "tag=2";
}

function addSwitch() {
	params = new Array(0,-1);//"0"说明是新增操作
	window.showModalDialog("alarmSwitchInfoGX.jsp",window,"dialogWidth=600px;dialogHeight=666px;help=0;scroll=0;status=0;");
}
