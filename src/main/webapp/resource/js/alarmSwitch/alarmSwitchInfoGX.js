...
function linkToRule() {
	...
	window.showModalDialog("/workshop/alarmrule/alarmRuleList.html?fromNe=1&ruleTypeId=0&neId=" + neId,"网元相关告警规则页面","help=0;scroll=yes;status=0;dialogWidth=1000px;dialogHeight=600px");
}
...
function syncKPI()
{
	...
	if(currentNeId)
	{
		...
		if(isSuccess(xmlhttp)) 
		{
			...
			if(differentNeId!="")
			{
				if(QMsg("网元:" + differentNeId + " 的网元类型与所选网元类型不一致,是否同步其他网元")!=MSG_YES)
				{
					return false;
				}
			}
