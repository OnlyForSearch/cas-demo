...
{  
    ...
    var sMainPageURL=(oMenuJS==null)?"/index_ZJ.jsp":oMenuJS.mainPage;
	window.top.location.replace("/index_ZJ.jsp");
}
...
function doMenu_loadTree(frameName,mainFrame,mainUrl) 
{
    ...
    var sSecondPageURL="http://134.96.93.182:7001/indexFrame.jsp";
	var menuId="-1";
    var arrayUrl=getURLSearch();
    if(arrayUrl!=null)
    {
	  var menuId = arrayUrl.menuId ? arrayUrl.menuId : menuId;
	}
...
function doMenuFuncDesc()
{
    ...
    alert(sSecondPageURL+"?id="+oSelectedItem.id+"&mainUrl="+encodeURIComponent(mainLoadUrl)+"&menuId="+menuId+"&from=itsm")
	top.window.location.replace(sSecondPageURL+"?id="+oSelectedItem.id+"&mainUrl="+encodeURIComponent(mainLoadUrl)+"&menuId="+menuId+"&from=itsm");
}
...
function doMenu_open(width,height)
{
	...
	{
		doMenu_open_by_url("http://134.96.93.182:7001/" + oSelectedItem.SERVER_URL_NAME,width,height,oSelectedItem.id);
	}
}
...
function doMenu_loadFullWin(isSlider)
{
    ...
    if(isSlider)
    {
       sURL="http://134.96.93.182:7001/workshop/topo/slider.html?url="+encodeURIComponent(oSelectedItem.SERVER_URL_NAME);
    }
    if(sURL)
	{
		window.open("http://134.96.93.182:7001/" + sURL,null, "status=no,toolbar=no,menubar=no,location=no,fullscreen=1");
	}
}
...
{
	window.top.document.all(frameName).src = "http://134.96.93.182:7001/" + oSelectedItem.SERVER_URL_NAME;
}
