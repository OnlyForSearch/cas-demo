...
function addTopMenu(id, oMenuBar, curMenuId) {
	xmlDoc.loadXML(getHtmlData("addTopMenuData"));
...
function doMenu_loadTree(frameName,mainFrame,mainUrl) 
{
    ...
    if(arrayUrl!=null)
    {
    	if(typeof(arrayUrl.menuId) != "undefined"){
...
{
	top.window.location.href = oSelectedItem.SERVER_URL_NAME;
}

function oaLoginEda(width,height){
	url=oSelectedItem.SERVER_URL_NAME.concat("&username="+userName);
	doMenu_open_by_url(url,width,height,oSelectedItem.id);
