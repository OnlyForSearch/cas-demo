//引用formValidatorRegex.js内容，合二为一
var regexEnum = 
{
	intege:"^-?[1-9]\\d*$",					//整数
	intege1:"^[1-9]\\d*$",					//正整数
	...
	//扩展
	intege_other:"^([1-9]\\d*|0)$",					//整数
	mobile_other: "^(134|135|136|137|138|139|150|151|152|154|157|158|159|187|188)[0-9]{8}$",//移动
	rg_mobile:"^(13|15|18)\\d{9}([,，](13|15|18)\\d{9})?$",			//手机号码(移动/联通/电信)
	rg_fax:"^((0\\d{2,3}|00852|00853|00886)-\\d{7,8}(-\\d{1,4})?)([,，](0\\d{2,3}|00852|00853|00886)-\\d{7,8}(-\\d{1,4})?)?$",	//传真的函数 [ 区号-电话(-分机)]
	rg_phone:"^((0\\d{2,3}|00852|00853|00886)-\\d{7,8}(-\\d{1,4})?)([,，](0\\d{2,3}|00852|00853|00886)-\\d{7,8}(-\\d{1,4})?)?$"	//电话号码的函数 [ 区号-电话(-分机)]
}
...
$.formValidator = 
{
	...
	initConfig : function(controlOptions)
	{
		var settings = 
		{
			...
			errorfocus:true,
			wideword:true,
			validatormode:""||"div_mode", //验证模式 div_mode 用层来提示消息, text_mode 将显示错误文本框
			path:"" //工程路径
		};
	...
	//设置文本框状态样式
	setElemState : function(textid,path,showclass,showmsg){
		$('#'+textid).addClass(showclass);
		$('#'+textid+'Info').html("&nbsp;<img src='"+path+"/jquery/validator/images/err.gif' class='infoImg' title='"+showmsg+"' />");
	},
	
	resetElemState : function(returnObj){
		var id = returnObj.id;
		$('#'+id).removeClass('errText');
		$('#'+id+'Info').html('');
	},
	...
	//update by cwself, update date 2009-9-23 显示错误消息
	showInfo : function(returnObj){
		var id = returnObj.id;
		var elem = $("#"+id).get(0);
		var type = elem.type;
		var srcTag = elem.tagName;
		...
		var settings = $("#"+id).get(0).settings;
		var intiConfig = $.formValidator.getInitConfig(settings[0].validatorgroup);
		var path = intiConfig.path;
		if (!isvalid){
			showclass = "errText";
			showmsg = (returnObj.errormsg==""? setting.onerror : returnObj.errormsg);
			$.formValidator.setElemState(id,path,showclass,showmsg);
		}
		else{
			$.formValidator.resetElemState(returnObj);
		}
		return showmsg;
	},
        ...
        switch(sType)
		{
			...
			case "file":
				...
				if (initConfig.wideword)
				{
					...
					{
						/////////////////////////////////////////////////////
						if (val.charCodeAt(i) == 12288) {  //全角空格
							len += 2;
						} else if (val.charCodeAt(i) > 65280 && val.charCodeAt(i) < 65375) {  //全角符号
							len += 2;
						} else if (val.charCodeAt(i) >= 0x4e00 && val.charCodeAt(i) <= 0x9fa5){ 
							len += 2;
						} else {
							len++;
						}
...
//here							alert(returnObj.id);
							if(initConfig.validatormode == "div_mode"){
								$.formValidator.showMessage(returnObj);
							}
							if(initConfig.validatormode == "text_mode"){
								$.formValidator.resetElemState(returnObj);
								$.formValidator.showInfo(returnObj);
							}
						}
					}
				}
		...
		if(srcTag == "input" || srcTag=="textarea")
		{
			...
			jqobj.bind(setting.triggerevent, function(){
				...
				else
				{
					...
					else if(initConfig.validatormode == 'text_mode'){
						showmsg = $.formValidator.showInfo(returnObj);
					}
					if(!returnObj.isvalid)
					{
						//自动修正错误
...
function showErrMsg(infoId, msg, infoStyle){
	infoStyle = infoStyle || {};
	$('#'+infoId).html(msg);
	$('#'+infoId).css(infoStyle);
	$('#'+infoId).show();
}

function cleanErrMsg(infoId){
	$('#'+infoId).hide();
}
