...
//	var mainLoadUrl = oSelectedItem.CUSTOM_URL_NAME || (mainUrl || defaultWelComePage);
	//	window.location.replace(sSecondPageURL+"?id="+oSelectedItem.id+"&mainUrl="+encodeURIComponent(mainLoadUrl)+"&menuId="+menuId+"&action="+action);
	//	window.top.document.all("menuFrame").src = "menu.html?id="+oSelectedItem.id;
	
	if(id==""){
		window.location.replace("indexFrame.jsp?id="+oSelectedItem.id+"&menuId="+menuId);		
	}else{
		var sMenuName = oSelectedItem.innerText;
		var sMenuId   = oSelectedItem.id;
		//addTopMenu_NOC(menuId, document.getElementById("oMenuBar"), oSelectedItem.id);
		var conObj = document.getElementById("oMenuBar");
		var menuItems = conObj.getElementsByTagName("SPAN");
		for(var i=0; i<menuItems.length; i++){
   			var aMenuBg=getaMenuBg_NOC();
			if(menuItems[i].className == aMenuBg.menuSeledClass){
				menuItems[i].className = aMenuBg.menuNormalClass;
			}

			if(menuItems[i].id == sMenuId){
   				var aMenuBg=getaMenuBg_NOC();
   				menuItems[i].className = aMenuBg.menuSeledClass;
			}
		}
		
		//window.top.document.all("menuFrame").src = "menu.html?id="+sMenuId+"&menuName="+encodeURIComponent(sMenuName);
		top.menuFrame.oDiv.innerHTML = sMenuName;
		top.menuFrame.loadMenu(oSelectedItem.id);
    	window.top.frames["mainFrameset"].cols = "195,7,*";
	}
}
...
function doMenu_loadWin(frameName)
{
	...
	top.tabWinTitle.openWin(oSelectedItem.SERVER_URL_NAME,oSelectedItem.PRIVILEGE_NAME,oSelectedItem.PRIVILEGE_ID); 	//四川NOC
}
//删除快捷菜单
