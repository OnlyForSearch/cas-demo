...
function getNeighborDayLast(oDateTime,durings){
    ...
    var xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
    if(oDateTime)
        sql=encodeURIComponent("select to_char(to_date('"+oDateTime+"','yyyy-mm-dd hh24:mi:ss')"+durings+",'yyyy-mm-dd')||' 23:59:59' from dual");
    else
        sql=encodeURIComponent("select to_char(sysdate"+durings+",'yyyy-mm-dd')||' 23:59:59' from dual");
    xmlhttp.Open("POST","/servlet/@Deprecated/ExecServlet?action=101&paramValue="+getAESEncode(sql),false);
    xmlhttp.send();
    if (isSuccess(xmlhttp))
    {
...
function getCurrentMonthFirstDay(){
    var date,sql;
    var xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
    sql=encodeURIComponent("select to_char(trunc(sysdate,'mm'),'yyyy-mm-dd')||' 00:00:00' from dual");
    xmlhttp.Open("POST","/servlet/@Deprecated/ExecServlet?action=101&paramValue="+getAESEncode(sql),false);
    xmlhttp.send();
    if (isSuccess(xmlhttp))
    {
...
function changeCycle(i,type,callback,obj){
  ...
  // obj.style.color='blue';
   ...
   switch(type){
      case 't':
      case 'T':
           document.getElementById("begin_time_"+i).innerHTML=getModifyDateTime("-6/24");
           document.getElementById("end_time_"+i).innerHTML=getModifyDateTime("+0"); 
      ...
      case 'd':
      case 'D':
           document.getElementById("begin_time_"+i).innerHTML= getCurServerDateTime(false)+" 00:00:00";
           document.getElementById("end_time_"+i).innerHTML= getCurServerDateTime(true);
           document.getElementById("pre_cycle_"+i).innerHTML="前一天";
           document.getElementById("new_cycle_"+i).innerHTML="今天";
           document.getElementById("last_cycle_"+i).innerHTML="后一天";
      ...
      case 'm':
      case 'M':
           document.getElementById("begin_time_"+i).innerHTML = getCurrentMonthFirstDay();
           document.getElementById("end_time_"+i).innerHTML= getCurServerDateTime(true);
...
function getLastResult(type,i,callback,obj){
   ...
   //obj.style.color='blue';
   bonGlobal.cycles[i] = obj;
   var beginEle = document.getElementById("begin_time_"+i);
   var endEle = document.getElementById("end_time_"+i);
  switch(type){
      case 't':
      case 'T':
           beginEle.innerHTML= getModifyDateTime("-6/24");
           endEle.innerHTML= getModifyDateTime("+0"); 
           break;
      case 'd':
      case 'D':
           beginEle.innerHTML= getCurServerDateTime(false)+" 00:00:00";
           endEle.innerHTML= getCurServerDateTime(true);
           break;
      case 'm':
      case 'M':
           beginEle.innerHTML = getCurrentMonthFirstDay();
           endEle.innerHTML= getCurServerDateTime(true);
           break;
  }
  document.getElementById("last_begin_time_"+i).value= beginEle.innerText;
  document.getElementById("last_end_time_"+i).value= endEle.innerText;
  callback(document.getElementById("last_begin_time_"+i).value,document.getElementById("last_end_time_"+i).value);
}
...
function changeResult(type,i,durings,callback,obj){
   ...
   var endEle = document.getElementById("end_time_"+i);
    switch(type){
      case 't':
      case 'T':
           beginEle.innerHTML=getModifyDateTime(durings, beginEle.innerText);
           endEle.innerHTML=getModifyDateTime(durings,endEle.innerText);         
           break;
      case 'd':
      case 'D':
           beginEle.innerHTML= getNeighborDayBegin(beginEle.innerText,durings);
           endEle.innerHTML = getNeighborDayLast(endEle.innerText,durings);
           break;
      case 'm':
      case 'M':
           beginEle.innerHTML= getNeighborMonthFirstDay(beginEle.innerText,durings);
           endEle.innerHTML=getNeighborMonthLastDay(endEle.innerText,durings);
           break;
  }  
  document.getElementById("last_begin_time_"+i).value=beginEle.innerText;
  document.getElementById("last_end_time_"+i).value= endEle.innerText;
...
function showSearchWin(obj,i){
    ...
    ele.style.left = (document.body.scrollLeft||document.documentElement.scrollLeft)+obj.getBoundingClientRect().left-210;
    ele.style.top =  (document.body.scrollTop||document.documentElement.scrollTop)+obj.getBoundingClientRect().top+30;
}

function selfDefineSearch(callback,i){
     document.getElementById("begin_time_"+i).innerHTML= document.getElementById("last_begin_time_"+i).value;
     document.getElementById("end_time_"+i).innerHTML= document.getElementById("last_end_time_"+i).value;
...
function getPageMenu(obj_div,callback,menuIds,timeType,i,noShowTime,showRefresh){
  if(!i){
     i=0;
  }
  var bodyHtml="<div id='searchWin_"+i+"' style='position:absolute;display:none;'>";
  bodyHtml+="<div class='b1'></div>";
  bodyHtml+="<div class='b2'></div>";
  bodyHtml+="<div class='b3'></div>";
  bodyHtml+="<div class='searchWin' style='width:275px;padding-top:10px;padding-bottom:10px;' >";
  bodyHtml+="<table style='width:100%'><tr><td align='right'><span class='searchWinColor'>查询时间：从</span></td><td><input id='last_begin_time_"+i+"' type='text' style='width:185px;' onFocus=\"WdatePicker({isShowClear:true,dateFmt:'yyyy-MM-dd HH:mm:ss', readOnly:true})\" /></td></tr>"+
               "<tr><td align='right'><span class='searchWinColor'>到</span></td><td><input id='last_end_time_"+i+"' type='text' style='width:185px;' onFocus=\"WdatePicker({isShowClear:true,dateFmt:'yyyy-MM-dd HH:mm:ss', readOnly:true})\"/></td></tr>"+
               " <tr><td></td><td><div onMouseOver=\"this.className='flatButOverS'\"  onMouseOut=\"this.className='flatButS'\"  onclick='selfDefineSearch("+callback+","+i+")'  class='flatButS'>查&nbsp;询</div>"+
               "<div onMouseOver=\"this.className='flatButOverS'\"  onMouseOut=\"this.className='flatButS'\" onclick='hideSearchWin("+i+")' class='flatButS'>取&nbsp;消</div>"+
               "</td></tr></table>";
  bodyHtml+="</div>";
  bodyHtml+="<div class='b4'></div>";
  bodyHtml+="<div class='b5'></div>";
  bodyHtml+="<div class='b6'></div>";
  bodyHtml+="</div>";
  document.body.insertAdjacentHTML('afterBegin',bodyHtml);
  var menuTypeHtml='';
  menuTypeHtml+="<div style='float:left;margin-left:5px;line-height:28px;height:28px;margin-top:5px;margin-right:10px;";
  if(noShowTime){
   menuTypeHtml+="display:none";
  }
  menuTypeHtml+="'><span class='timeColor'>从&nbsp;</span><span id='begin_time_"+i+"' class='timeColor'></span><span class='timeColor'>&nbsp;到&nbsp;</span><span id='end_time_"+i+"'  class='timeColor'></span></div>";  
  ...
  switch(timeType){
      ...
      case 'T':
           ...
           if(showRefresh)
           menuTypeHtml +="<div onMouseOver=\"this.className='flatButOver'\"  onMouseOut=\"this.className='flatBut'\"  onclick='refreshData("+i+","+callback+")'  class='flatBut'>刷新</div>";
      ...
      case 'D':
           ...
           if(showRefresh)
           menuTypeHtml +="<div onMouseOver=\"this.className='flatButOver'\"  onMouseOut=\"this.className='flatBut'\"  onclick='refreshData("+i+","+callback+")'  class='flatBut'>刷新</div>";
           menuTypeHtml +="<div onMouseOver=\"timeOver(this,"+i+")\"  onMouseOut=\"timeOut(this,"+i+")\"  id='pre_cycle_"+i+"' class='flatBut'>前一天</div>";
           menuTypeHtml +="<div onMouseOver=\"timeOver(this,"+i+")\"  onMouseOut=\"timeOut(this,"+i+")\"  id='new_cycle_"+i+"' class='flatBut'>今天</div>";
           menuTypeHtml +="<div onMouseOver=\"timeOver(this,"+i+")\"  onMouseOut=\"timeOut(this,"+i+")\"  id='last_cycle_"+i+"' class='flatBut'>后一天</div>";
           menuTypeHtml +="<div onclick='showSearchWin(this,"+i+")' class='flatBut'>自定义查询</div>";  
           break;
      case 'm':
      case 'M':
...
function getModifyDate(duringDays,oDate){
    ...
    var xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
    if(oDate)
        sql=encodeURIComponent("select to_char(to_date('"+oDate+"','yyyy-mm-dd')"+duringDays+",'yyyy-mm-dd') from dual");
    else
        sql=encodeURIComponent("select to_char(sysdate"+duringDays+",'yyyy-mm-dd') from dual");
    xmlhttp.Open("POST","/servlet/@Deprecated/ExecServlet?action=101&paramValue="+getAESEncode(sql),false);
    xmlhttp.send();
    if (isSuccess(xmlhttp))
    {
...
//获取控制命令参数
 function getControlParam(controlId,insId){
    ...
    var retJsonContent='';
    var xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
    sql=encodeURIComponent("select t.param_name,decode(t.comp_id,2,(select to_CHAR(sysdate,'yyyymmdd') from dual),d.param_value) param_value from CI_CTRL_COL_PARAM t,CI_CTRL_COL_PARAM_INSTANCE d,CI_CTRL_INSPECT a where t.param_id=d.param_id and t.ci_ctrl_col_id="+controlId+" and d.inspect_id=a.inspect_id and a.instance_id="+insId);
    xmlhttp.Open("POST","/servlet/@Deprecated/ExecServlet?action=101&paramValue="+getAESEncode(sql),false);
    xmlhttp.send();
    if (isSuccess(xmlhttp))
    {
 ...
 function refreshData(i,callback){
   callback(document.getElementById("last_begin_time_"+i).value,document.getElementById("last_end_time_"+i).value);
}
