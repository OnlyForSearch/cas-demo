function drawChart(chartUrl, chartObj){if(typeof chartObj == "string") chartObj = ase_8(chartObj);ase_r(chartUrl,function(chartText){chartObj.innerHTML = chartText;},function(){});}function _loadJsChartViewer(chartName){if (!window.JsChartViewer) return;var viewer = JsChartViewer.get(chartName);if(viewer.getCustomAttr("_FMW_CHART_IS_ZOOM_CHART")!="true") return;if(JsChartViewer.canSupportPartialUpdate()){viewer.attachHandler("ViewPortChanged", viewer.partialUpdate);}viewer.setMouseUsage(JsChartViewer.Scroll);viewer.setScrollDirection(JsChartViewer.HorizontalVertical);viewer.setZoomDirection(JsChartViewer.HorizontalVertical);}
//function drawChart(chartUrl, chartObj){if(typeof chartObj == "string") chartObj = ase_8(chartObj);var chartName = "_FMW_CHART_" + new Date().getTime();ase_r(ase_g(chartUrl, "_FMW_CHART="+chartName),function(chartText){_loadJsChartViewer(chartObj, chartName, chartText, chartUrl);},function(){});}function _loadJsChartViewer(chartObj, chartName, chartText, chartUrl){var startIndex = chartUrl.indexOf("/");startIndex = (startIndex!=-1) ? startIndex : chartUrl.indexOf("\\");startIndex = (startIndex!=-1) ? startIndex: 0;var endIndex = chartUrl.indexOf("?");endIndex = (endIndex!=-1) ? endIndex: chartUrl.length;var chartUrl = chartUrl.substring(startIndex, endIndex);chartText = ase_h(chartText, chartUrl, chartUrl+"?");chartObj.innerHTML = chartText;if ( ! window.JsChartViewer) return;var viewer = JsChartViewer.get(chartName);if(viewer.getCustomAttr("_FMW_CHART_IS_ZOOM_CHART")!="true") return;if (JsChartViewer.canSupportPartialUpdate()){viewer.attachHandler("ViewPortChanged", viewer.partialUpdate);}viewer.setMouseUsage(JsChartViewer.Scroll);viewer.setScrollDirection(JsChartViewer.HorizontalVertical);viewer.setZoomDirection(JsChartViewer.HorizontalVertical);}
...
function ase_q(){if(typeof XMLHttpRequest!='undefined')return new XMLHttpRequest();var _XMLHTTP = ["Msxml2.XMLHTTP.6.0", "Msxml2.XMLHTTP.5.0", "Msxml2.XMLHTTP.4.0", "MSXML2.XMLHTTP.3.0", "MSXML2.XMLHTTP", "Microsoft.XMLHTTP"];var returnValue;for (var i = 0; i < _XMLHTTP.length; i++){try{returnValue = new ActiveXObject(_XMLHTTP[i]);break;}catch (e){}}return returnValue;}
...
_jcvp=_jcv.prototype;_jcv.l22=function(lc1){var ld1=window.cdjcv_path;if(typeof ld1=="undefined")ld1=ase_i("fmw_chart.js");else if((ld1.length>0)&&("/=".indexOf(ld1.charAt(ld1.length-1))==-1))ld1+='/';return ld1+lc1;}
